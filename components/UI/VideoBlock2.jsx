      ...
      {hasWindow && 
        ...
        >
          <div className='absolute xl:w-[1200px] top-0 left-0 xl:left-1/2 xl:-translate-x-1/2 flex flex-row xl:flex-wrap items-center justify-start xl:justify-between px-6 sm:px-10'>
            {videos.map((video) => (
              <div key={video.id} className={`relative w-[200px] h-[432px] flex flex-col items-center justify-center overflow-hidden rounded-[15px] group mr-8 last:mr-0 xl:mr-10 mt-10 xl:last:mr-10 xl:mt-0 xl:mb-10 border border-solid cursor-pointer`}>
                  <ReactPlayer
                    url={video.href}
                    width={"100%"}
                    height={"100%"}
